      ...
      <div class="el-game-wrapper__content">
        <div class="el-container-wrap">
          <div class="ele_container">
            <div class="ele_wrap">
              <div class="tabs">
                <template v-for="txxdCategory in txxdCategories">
                  <div :class="['ele-item', 'tab', currentTab === txxdCategory.api_name ? 'active' : '']"
                    @click="() => handleActiveTab(txxdCategory.api_name)">
                    <div class="l-icon joker">
                      <img :src="txxdCategory.web_pic">
                    </div>
                  </div>
                </template>
              </div>
            ...
            <div class="ele_container">
              <div class="ele_wrap">
                <div class="tabs">
                  <template v-for="txxdCategory in txxdCategories">
                    <div
                      :class="['ele-item', 'tab', 'jill-game-tab', 'js-get-text', currentTab === txxdCategory.api_name ? 'active' : '']"
                      @click="() => handleActiveTab(txxdCategory.api_name)">
                      <div class="l-icon joker">
                        <img :src="txxdCategory.mobile_pic">
                      </div>
                    </div>
                  </template>
                </div>
...
<script setup lang="ts">
  ...
  import { useTxxd } from '../txxd.model'
  ...
  if (txxdCategories.value?.length > 0) {
    txxdStore.fetchGameTxxds({
      ...
      gameType: 50,
      tag: 'all',
      api_code: txxdCategories.value[0]?.api_name,
      keyword: '',
      page: 1,
      limit: 18,
    })
