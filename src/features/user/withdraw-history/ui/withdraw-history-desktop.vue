<template>
  ...
  <v-data-table-server class="py-4" v-model:items-per-page="state.limit" :headers="headers"
    :items="withdrawHistories.data" :items-length="withdrawHistories.total" :items-per-page-options="[10, 25, 50, 100]"
    items-per-page-text="Số kết quả trên một trang" no-data-text="Không có dữ liệu"
    @update:options="handleUpdatePagination"></v-data-table-server>
</template>
<script setup lang="ts">
  import type { TWithdrawHistories, TWithdrawHistoryStatus } from '../withdraw-history.type';

  type TProps = {
    withdrawHistories: TWithdrawHistories;
    statusOptions: Array<TWithdrawHistoryStatus>;
  ...
  const state = reactive<TState>({
    limit: props.withdrawHistories.per_page || 10,
  })
  const headers = [
    {
      title: 'Số serial',
  ...
  const handleUpdatePagination = ({ page, itemsPerPage }: { page: number, itemsPerPage: number }) => {
